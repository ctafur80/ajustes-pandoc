...
En lo que respecta a @costa[p.~11] *Definición 1.1 (Métrica)*, me gusta más
la siguiente, ya que me parece más "elegante".

#definition(title: "Definición de Métrica")[
  Dado un conjunto $bold(P)$ no vacío, una _métrica_ (también llamada
  _distancia_) es toda aplicación $d: bold(P) times bold(P) arrow.r bb(R)^+
  union {0}$ en la que para cualesquiera $x, y, z in bold(P)$ se cumplen:

  + $d(x, y) = 0 med arrow.l.r.double med x = y$.
  + $d(x, y) = d(y, x)$. (Simetría.)
  + $d(x, y) lt.eq d(x, z) + d(z, y)$. (Desigualdad triangular.)
] <def-metrica>

Tal y como se explica en la #link(<def-metrica>)[definición de métrica], se
ve que TKTK.
...
En @costa[p.~11] *Ejemplo 1.2* no llega a demostrar, ni aquí ni en el
*Ejercicio 1.2*, los dos primeros puntos. Aunque sea fácil, vamos a hacerlo
aquí.
...
En @costa *Ejemplo 1.3*, lo más relevante está en que nos podemos basar en
la desigualdad triangular en $(bb(R), +)$ para demostrar que se cumple el
punto 3 en la Definición 1.1 (Métrica). Esta se da como conocimiento básico,
es decir, como prerrequisito.

Aunque el *Teorema 1.4* (de la Métrica Inducida, @costa[p.~12]) pueda
...
$ delta:  & M times M & arrow.r     & bb(R) \
          & (x, y)    & arrow.r.bar & delta (x, y) $

mientras que

$ delta\|_(M' times M'): & M' times M' & arrow.r & bb(R) \
    & (x, y) & arrow.r.bar & delta\|_(M' times M') (x, y) = delta (x, y) $
...
En @costa *Definición 1.5*, alternativamente a como se define el concepto de
_isometría_, podríamos definirla como

$ g: med (M, delta) arrow.r (M', delta') $
...
$ delta_G = "Distancia máxima entre " p " y " q $
...
estaremos demostrando que $[a, a] subset.eq {a}$. Si se va en el
otro sentido, que ${a} subset.eq [a, a]$.
...
$ g(0) = (f_1^(-1) compose f_2) (0) = f_1^(-1) (f_2 (0)) = f_1^(-1) (f_1 (0)) = "id"_(bb(R)) (0) = 0 $

y

$ g(1) = (f_1^(-1) compose f_2) (1) = f_1^(-1) (f_2 (1)) = f_1^(-1) (f_1 (1)) = "id"_(bb(R)) (1) = 1 $
...
Esto se debe a que deseamos, por un lado, que $g (0) = h (0)$. Para que se
dé esto, debe cumplirse lo siguiente:
...
Por cierto, advierta que $|h (1) - h (0)| = 1$ por ser $h$ una isometría.
Por tanto, $sigma in {-1, 1}$.
