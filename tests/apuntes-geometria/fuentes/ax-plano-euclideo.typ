...
El Axioma P1 nos proporciona la clase de entidad básica de la geometría
euclidiana: el _punto_ (_point_). Este no es más que como se llama a los
elementos del espacio métrico en esta geometría en concreto.

Evidentemente, para los puntos se usará la notación de los elementos. Es
decir, cosas como $X in bold(P)$. Otros conjuntos más complejos de elementos
reciben, en general, la designación de _figuras_ (_figures_). Evidentemente,
verá cosas como $r subset.eq bold(P)$ para las figuras.
...
En la definición de _recta_ en @costa[p.~23], el punto i se incluye para que
un conjunto formado por un único punto no cumpla las condiciones para ser
una recta.

Algo similar sucede con el punto i del Axioma P2 en @costa[p.~24]. De no
...
Tal y como se explica en la *Observación 2.4* en @costa[p.~24], en realidad
en el punto ii del Axioma P2 se podría haber limitado a decir que entre dos
puntos no coincidentes siempre hay (al menos) una recta; que sea única es
algo que se puede deducir y, por tanto, se podría haber presentado como un
teorema. Veamos la demostración de este.

#proof[
  Suponemos que existen dos rectas $r$ y $s$ que contienen a dos puntos $A,
  B in bold(P)$.

  Dado un punto arbitrario $X in bold(P)$ tal que $X in r$, por el punto ii
  de la Definición 2.2 (de Recta) se tiene que $A$, $B$ y $X$ están
  alineados.

  Desde otro punto de vista, sabemos que $A, B in s$. Entonces, por el punto
  iii de la Definición 2.2 (de Recta) tenemos que, como $A$, $B$ y $X$ están
  alineados, se da que $X in s$. Con este razonamiento, hemos llegado a
  demostrar que $r subset.eq s$.
...
#proof[
  ...
  // TODO Convertirlo a tabla como en el antiguo.

  - $p$: $r$ y $s$ no tienen ningún punto en común. Es decir,
    $r inter s = emptyset$
  - $q$: $r$ y $s$ se cortan. Es decir, existe un único $X in bb(P)$ tal
    que ${ X } = r inter s$ siendo ${ X } eq.not emptyset$.
  - $m$: $r$ y $s$ son la misma (son coincidentes). Es decir, $r = s$.
  - $n$: $r$ y $s$ son paralelas. Por definición del paralelismo de
    rectas, se tiene que $n arrow.l.r.double p or m$.

  Caso 1. Nuestra hipótesis es que $r$ y $s$ se cortan y son paralelas. La
  hipótesis será, por tanto, para este caso, la siguiente:

  $ q and n med arrow.l.r.double med q and (p or m) med arrow.l.r.double med
  (q and m) or (q and p) $
...
#block[
  ...
  isometría $gamma : (r, d) arrow.r (bb(R), d')$ siendo $d'$ la distancia
  definida del modo siguiente, para cualesquiera $x, y in bb(R)$,
...
#proof[
  ...
  con lo que $A' = gamma^(- 1) (2 b - a)$ será único por ser $gamma$
  biyectiva.
...
de pasar a ver sus propiedades, presentadas en el #strong[Teorema 2.12];,
hay un resultado que me gustaría presentar y que se deduce directamente de
este y que se usa en muchas demostraciones. Sería básicamente el recíproco
del punto 3 de este.
...
#block[
  ...
  distintos.
] <thm-ampliacion-ax-p4>

Tal y como se explica en el #link(<thm-ampliacion-ax-p4>)[] TKTK.
...
Semiplanos). En la 4, el primer paso se justifica concretamente por el punto
i del Axioma P2.
...
Axioma P4, tenemos que para todo $X in [P, Q]$ se tiene que $X in.not r$, o,
lo que es lo mismo, no existe $X in [P, Q]$ tal que $X in r$, cosa que
...
#theorem[
  ...
  prima ("$'$") para un elemento transformado, por ejemplo, $X' = g (X)$
  para un punto $X in bold(P)$, se tiene:
...
$ g (X) in g ([A, B]) quad arrow.l.r.double quad X in [A, B] $
...
#proof[
  ...
  parte del *Teorema 2.18*, se tiene que $g (M) in [A', B']$. Le recuerdo
  que usamos la notación $X' = g (X)$ para un punto cualquiera $X in
  bold(P)$, al igual que se hace en el libro.
  ...
  $ "medio"[A', B'] subset.eq ("medio"[A, B])' $

  Advierta que hemos "estirado" la notación, representando a "$g ("medio"[A,
  B])$" por "$("medio"[A, B])'$".
...
encadenando bicondicionales, "$arrow.l.r.double$", pero deseo que quede al
menos una explicación con mayor detalle, por si el lector no las entiende.
...
#proof[
  Lo primero que debe comprobarse es que $g (r)$ y $g (s)$ son rectas, por
  el Teorema 2.18.

  Evidentemente, $g (V) in g (r) inter g (s)$, ya que, por ejemplo, por
  darse $V in r$, se tiene que $g (V) in g (r)$.

  De $V in r inter s$ se deduce que $V in r$ y $V in s$, y, de esto, a su
  vez, que $g (V) in g (r)$ y $g (V) in g (r)$, o, lo que es lo mismo, $g
  (V) in g (r) inter g (s)$.
  ...
  Supongamos que existe, además de $g (V)$, otro punto $Z in g (r) inter g
  (s)$. Esto es lo mismo que afirmar que $Z in g (r)$ y $Z in g (s)$.
...
#theorem[
  ...
  $g(dash(r))$ es una de las semirrectas definidas por el punto $g (V)$ en
  la recta $g (r)$.
]
...
#proof[
  ...
  Al pertenecer $A$ a $H^1$, por el punto 1 del Axioma P4 se tiene que todo
  punto de $H^1$ está fuera de $r$. Por tanto, $A in.not r$.

  como tenemos un punto, $M$ que pertenece tanto a $r$ como a $s$ y un punto
  $A$ que pertenece a $s$ pero no a $r$, ambas rectas, es decir, $r$ y $s$,
  serán secantes, por aplicación del Teorema 2.7 (de las Posiciones
  Relativas de Dos Rectas). Entonces, también serán secantes el segmento
  $[A, A']$ y la recta $r$, es decir, se cumple que $[A, A'] inter r eq.not
  emptyset$. Aplicando ahora el Teorema Ampliación del Punto 3 del Axioma
  P4, tenemos que, como $A in H^1$, no queda otra más que $A' in H^2$.

  Por la Observación 2.3 y el punto ii del Axioma P2 (incluyendo la
  unicidad), sabemos que existe una única recta que contiene al segmento
  $[A, A']$. La designaremos por $s$.

  Al pertenecer $A$ a $H^1$, por el punto 1 del Axioma P4 se tiene que todo
  punto de $H^1$ está fuera de $r$. Por tanto, como tenemos un punto, $M$
  que pertenece tanto a $r$ como a $s$ y un punto $A$ que pertenece a $s$
  pero no a $r$, ambas rectas, es decir, $r$ y $s$, serán secantes, por
  aplicación del Teorema 2.7 (de las Posiciones Relativas de Dos Rectas).
  Entonces, también serán secantes el segmento $[A, A']$ y la recta $r$, es
  decir, se cumple que $[A, A'] inter r eq.not emptyset$. Aplicando ahora el
  Teorema Ampliación del Punto 3 del Axioma P4, tenemos que, como $A in
  H^1$, no queda otra más que $A' in H^2$.

  Ahora, nos fijaremos en que, por el punto 1 de la definición de _reflexión
  axial_, se tiene que $sigma (r) = r$. Aplicando entonces la última parte
  del Teorema 2.18 ---la que trata sobre los semiplanos---, tenemos que
  $H^1$ y $H^2$ son los semiplanos determinados por $sigma (r)$. Entonces,
  quedarían dos posibilidades únicamente: que los semiplanos transformados
  sean los mismos o que se intercambien. Del resultado anterior, es decir,
  de que si $A in H^1$ entonces $A' in H^2$, se tiene que se intercambian
  siempre, es decir,
